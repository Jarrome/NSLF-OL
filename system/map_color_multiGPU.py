...
from .data_color_thread import EncodeThread
...
class DenseIndexedMap:
    def __init__(self, model, args: argparse.Namespace,
                 latent_dim: int, devices, enable_async: bool = False,
        ...
        self.thread_nm_on_devices = np.zeros(len(self.devices))
    ...
    def allocate_block(self, idx: torch.Tensor, is_train: True):
        ...
        self.thread_nm_on_devices 
        for i in range(idx.shape[0]):
            device_id = np.argmin(self.thread_nm_on_devices)
            self.nslfs[idx[i].item()] = EncodeThread(idx[i].item(), self.devices[device_id], self.lr, is_train=is_train, mapping_model = self.args.mapping_model)

            self.thread_nm_on_devices[device_id] += 1
