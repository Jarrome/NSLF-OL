...
T_global = None
def vo_update(frame_data):#cur_frame, cur_depth):
    global ref_frame, ref_depth, kp_ref, d_ref, T_global

    calib = frame_data.calib
    fx, fy, cx, cy = calib.cx, calib.cy, calib.fx, calib.fy
    CIP = np.array([[fx,0,cx],[0,fy,cy],[0,0,1]])

    depth_scale = 1/calib.dscale



    cur_frame = (frame_data.rgb.cpu().numpy()*255).astype(np.uint8)
    cur_depth = frame_data.depth.cpu().numpy()
