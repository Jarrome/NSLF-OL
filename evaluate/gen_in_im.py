...
def save_depth_image(img_array, filepath):
...
if __name__ == '__main__':
    ...
    print('load_mesh...')
    # mesh
    assert vis_param.args.sequence_kwargs['mesh_gt'] != '', "Give a colored mesh!!!!!"
    print('from gt mesh')
    ...
    try:
      while True:
        ...
        # 2. predict
        st = time()
        pts, depth = vis_param.ray_caster.ray_cast(pose, return_depth=True) # N,3
        print('2. ray cast:', time()-st)
        invalid_mask = np.isinf(depth)
        depth = torch.tensor(depth).cuda().float()
        depth[invalid_mask] = 0.
        depth = depth.reshape((vis_param.ray_caster.H,vis_param.ray_caster.W))
        im = depth.cpu().detach().numpy()*4000.
        record_time = time()-st
        time_lst.append(record_time)
        print('3. color predict:', record_time)
        save_depth_image(im, os.path.join(vis_param.args.outdir,'depth','%d.png'%(id-1)))
