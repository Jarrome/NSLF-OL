...
if __name__ == '__main__':
    ...
    while True:
        ...
        mse = img2mse(color.detach(), aframe.rgb.detach())
        psnr = mse2psnr(mse).cpu().numpy()
        ...
        # 3. display
        surf = pygame.surfarray.make_surface(im.transpose(1,0,2)*255)
        display.blit(surf, (0, 0))
        pygame.display.update()
        print('4. display:', time()-st)
        if vis_param.sequence.frame_id >= len(vis_param.sequence): # finished
            print('Mean metrics:', "psnr", np.mean(metrics['psnr']))
        aframe = next(vis_param.sequence)
